...
export const PoolingStage: React.FC = () => {
  ...
  const frameCellSize = 312 / inputDim;
  const frameX = col * poolSize * frameCellSize;
  const frameY = row * poolSize * frameCellSize;
  const frameWidth = poolSize * frameCellSize;

  // Clamp bubble position so it never overflows the 312×312 canvas container
  const BUBBLE_W = 110;
  const BUBBLE_H = 24;
  const rawLeft = frameX + frameWidth / 2;
  const rawTop = frameY;
  const bubbleLeft = Math.min(Math.max(rawLeft - BUBBLE_W / 2, 2), 312 - BUBBLE_W - 2);
      ...
      <div className="flex flex-col md:flex-row items-center justify-center gap-20 w-full py-4">
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="relative p-1 border border-white/5 bg-[#161616] shadow-2xl">
            <canvas ref={inputCanvasRef} width={312} height={312} className="rounded-sm bg-black block" />
            <svg className="absolute inset-0 w-full h-full pointer-events-none z-20" viewBox="0 0 312 312">
        ...
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="relative p-1 border border-white/5 bg-[#161616] shadow-2xl">
            <canvas ref={outputCanvasRef} width={312} height={312} className="rounded-sm bg-black block" />
          </div>
        </div>
      </div>
    </div>
